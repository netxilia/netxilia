...
(function($) { // create private scope (inside you can use $ instead of jQuery)
			...
			if ( jQuery(this).is('input') && 'text' == jQuery(this).attr('type')) {
				...
				jQuery(this).click(function (ev) {
					...
					if (false == jQuery.data($this.get(0), "simpleDatepicker").hasDatepicker) {
						...
						if (initialDate) {
							var chosendate = Date.parseDate(initialDate, opts.format);
						...
						//ACR: add it in the same iframe as the input text 
						$this.parents("body").prepend(datepicker);
						
						// position the datepicker
						//var elPos = findPosition($this.get(0));
						//ACR: use offset
						var elPos = $this.offset();
						var x = (parseInt(opts.x) ? parseInt(opts.x) : 0) + elPos.left;
						var y = (parseInt(opts.y) ? parseInt(opts.y) : 0) + elPos.top;
	...
	jQuery.fn.simpleDatepicker.formatOutput = function (dateObj, opts) {
		...
		//ACR
		return dateObj.dateFormat(opts.format);
	};
